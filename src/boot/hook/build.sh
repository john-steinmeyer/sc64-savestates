...
[ "$(( MON_PAK_AT ))" -eq "$(( MON_BASE + 0x1000 ))" ] || { echo "ERROR: mon_pak_hit at $MON_PAK_AT, expected $(printf 0x%x $(( MON_BASE + 0x1000 )))" >&2; exit 1; }
...
[ "$(( $(secend .text) ))" -le "$(( MON_BASE + 0x1000 ))" ] || { echo "ERROR: the monitor's .text runs past +0x1000 (the pak server)" >&2; exit 1; }
[ "$(( $(secend .pak) ))" -le "$(( MON_BASE + 0x2000 ))" ] || { echo "ERROR: the pak server runs past +0x2000 (the load epilogue)" >&2; exit 1; }
...
[ "$(( MON_EPIL_AT ))" -eq "$(( MON_BASE + 0x2000 ))" ] || { echo "ERROR: mon_epi_load at $MON_EPIL_AT, expected $(printf 0x%x $(( MON_BASE + 0x2000 )))" >&2; exit 1; }
...
with open("hook_blob.h", "w") as f:
    ...
    f.write("#define SC64SS_HOOK_CFG_WORDS (28)\n\n")
